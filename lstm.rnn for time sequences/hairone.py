...
maotai = pd.read_csv('./hairdryer.csv')  # 读取股票文件

training_set = maotai.iloc[0:11112 - 3300, 16:17].values  # 前(2426-300=2126)天的开盘价作为训练集,表格从0开始计数，2:3 是提取[2:3)列，前闭后开,故提取出C列开盘价
test_set = maotai.iloc[11112 - 3300:, 16:17].values  # 后300天的开盘价作为测试集
...
for i in range(1000, len(training_set_scaled)):
...
x_train = np.reshape(x_train, (x_train.shape[0], 1000, 1))
# 测试集：csv表格中后300天数据
# 利用for循环，遍历整个测试集，提取测试集中连续60天的开盘价作为输入特征x_train，第61天的数据作为标签，for循环共构建300-60=240组数据。
for i in range(1000, len(test_set)):
    x_test.append(test_set[i - 1000:i, 0])
    y_test.append(test_set[i, 0])
# 测试集变array并reshape为符合RNN输入要求：[送入样本数， 循环核时间展开步数， 每个时间步输入特征个数]
x_test, y_test = np.array(x_test), np.array(y_test)
x_test = np.reshape(x_test, (x_test.shape[0], 1000, 1))
...
checkpoint_save_path = "./checkpointhairrnnbasic/rnn_stock.ckpt"
model = tf.keras.models.load_model('my_modelhairrnnbasic.h5')
'''
...
file = open('./weightshairrnnbasic.txt', 'w')  # 参数提取
...
model.save('my_modelhairrnnbasic.h5')
'''
# model_load
...
for i in range(2300):
    ...
    a = np.squeeze(a)
    x_test[0, 0:999, 0] = x_test[0, 1:1000, 0]  # 后者其实是1-60
    x_test[0, 999, 0] = a
...
real_stock_price = sc.inverse_transform(test_set[999:])
# 画出真实数据和预测数据的对比曲线
...
# 对真实数据还原---从（0，1）反归一化到原始范围
real_stock_price = sc.inverse_transform(training_set_scaled[1000:])
# 画出真实数据和预测数据的对比曲线
