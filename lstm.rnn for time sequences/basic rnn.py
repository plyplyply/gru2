...
for i in range(200, len(test_set)):
    x_test.append(test_set[i - 200:i, 0])
...
checkpoint_save_path = "./checkpoint2/rnn_stock.ckpt"
...
history = model.fit(x_train, y_train, batch_size=64, epochs=9, validation_data=(x_test, y_test), validation_freq=1,
                    callbacks=[cp_callback])

#model.summary()

file = open('./weights2.txt', 'w')  # 参数提取
...
for i in range(100):
    ...
    a = np.squeeze(a)
    x_test[0, 0:199, 0] = x_test[0, 1:200, 0]  # 后者其实是1-60
    x_test[0, 199, 0] = a
...
real_stock_price = sc.inverse_transform(test_set[200:])
# 画出真实数据和预测数据的对比曲线
...
# 对真实数据还原---从（0，1）反归一化到原始范围
real_stock_price = sc.inverse_transform(training_set_scaled[200:])
# 画出真实数据和预测数据的对比曲线
